""""
input: torques output: velocity under nnodely platform
...
Cs0  = 0# Parameter('Cs0', values=0)  
Cs1  = 1# Parameter('Cs1', values=1)  
Cs2  = 1# Parameter('Cs2', values=1)  
Cs3  = 0# Parameter('Cs3', values=0)  

#thetadot1_bef = Cs0 + (Cs1 * Integrate(thetaddot1, int_name ="dtheta1_init", method="euler")) #Trapezoidal

thetadot1_bef =    Integrate(thetaddot1, int_name ="dtheta1_init", method="euler") #Trapezoidal
...
thetadot1 = Output('thetadot1', thetadot1_bef)
thetadot2 = Output('thetadot2', thetadot2_bef)

# Model container
model = Modely(seed=0)

model.addModel('thetadot1', thetadot1)
model.addModel('thetadot2', thetadot2)


# Objectives

model.addMinimize('thetadot1-error', thetadot1_d, thetadot1, 'mse')
model.addMinimize('thetadot2-error', thetadot2_d, thetadot2, 'mse')
...
data_struct =  ['step', 'T1_data','T2_data','theta1_data', 'theta2_data',
...
result = model(sample, sampled=True, prediction_samples=(WINDOW//1))
# result = model(sample, sampled=True, num_of_samples=WINDOW, prediction_samples=100)

# 4) Convert to numpy arrays and flatten

y1_true = np.array(sample['thetadot1_data']).reshape(-1)
y1_pred = np.array(result['thetadot1']).reshape(-1)
y2_true = np.array(sample['thetadot2_data']).reshape(-1)
y2_pred = np.array(result['thetadot2']).reshape(-1)
...
# ---- Plot for thetadot2 ----
plt.figure()
plt.plot(x, y2_true, label='thetadot2_data (true)')
plt.plot(x, y2_pred, '--', label='thetadot2 (pred)')
plt.xlabel('sample index')
plt.ylabel('thetadot2')
plt.legend()
plt.title('thetadot2: true vs predicted')
plt.grid(True)


# ---- Plot for thetadot1 ----
plt.figure()
plt.plot(x, y1_true, label='thetadot1_data (true)')
plt.plot(x, y1_pred, '--', label='thetadot1 (pred)')
plt.xlabel('sample index')
plt.ylabel('thetadot1')
plt.legend()
plt.title('thetadot1: true vs predicted')
plt.grid(True)
